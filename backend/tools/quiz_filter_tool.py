...
def build_match_reason(product: Dict[str, Any], occasion: Optional[str], vibe: Optional[List[str]]) -> str:
    ...
    product_vibe = product.get("vibe") or []
...
def filter_products(
    ...
    """
    Hard-filter product query for the quiz flow. occasion / order_type / silhouette / vibe
    are structural and never dropped. If the full filter set returns zero results, loosens
    color first, then price, and reports which constraints were dropped so the UI can show
    a "results broadened" message.
    """
    try:
        loosened: List[str] = []
        current_color = color
